class Development < ApplicationRecord
  ...
  # Scope to find developments within a specified distance
  scope :within_distance_of, ->(lat, lon, distance_in_miles) do
    # Find addresses within the specified distance
    nearby_addresses = Address.near([lat, lon], distance_in_miles)

    # Get the development IDs of these addresses
    development_ids = nearby_addresses.map(&:addressable_id).compact
    # Find developments with these IDs
    where(id: development_ids)
  end
